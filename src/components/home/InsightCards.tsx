...
const SIG_CFG: Record<SignalValue, { label: string; text: string; bg: string; border: string }> = {
  "strong-buy":  { label: "Strong Buy",  text: "text-[#5edbb8]", bg: "bg-[#5edbb80a]", border: "border-[#5edbb833]" },
  "buy":         { label: "Buy",         text: "text-[#2ec898]", bg: "bg-[#2ec89808]", border: "border-[#2ec89828]" },
  ...
  "strong-sell": { label: "Strong Sell", text: "text-[#d04444]", bg: "bg-[#d044440a]", border: "border-[#d0444440]" },
};
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="h-1 rounded-full overflow-hidden bg-[#12121c]">
            <div className="h-full flex">
              <div className="bg-[#5edbb8]" style={{ width: `${pct(summary.strongBuys)}%` }} />
              <div className="bg-[#2ec898]" style={{ width: `${pct(summary.buys)}%` }} />
              <div className="bg-[#2a2a3e]" style={{ width: `${pct(summary.neutrals)}%` }} />
              <div className="bg-[#e05252]" style={{ width: `${pct(summary.sells)}%` }} />
              <div className="bg-[#d04444]" style={{ width: `${pct(summary.strongSells)}%` }} />
            </div>
          </div>
          <div className="flex items-center justify-between text-[11px]">
            <span className="text-[#2ec898]">{summary.strongBuys + summary.buys} Buy</span>
...
function qColor(v: number) { return v >= 70 ? "#2ec898" : v >= 45 ? "#7c7890" : "#e05252"; }
...
const STAGE_CFG: Record<Stage, { label: string; desc: string; color: string; pos: number }> = {
  emerging:  { label: "Emerging",  desc: "Story forming",     color: "#2ec898", pos: 0.12 },
...
function valLabel(pos: number) {
  if (pos < 0.25) return { text: "Cheap", color: "#2ec898" };
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="relative flex-1 h-1 rounded-full bg-[#12121c]">
        <div className="absolute inset-y-0 left-0 rounded-full opacity-20"
             style={{ width: `${pos * 100}%`, background: `linear-gradient(to right, #2ec898, #e05252)` }} />
      ...
      {isLoading ? <Skeleton lines={3} /> : txns.length > 0 ? (
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between pb-2 border-b border-[#1a1a28]">
            ...
            <span className={cn("text-xs font-semibold font-mono flex items-center gap-1", isNetBuy ? "text-[#2ec898]" : "text-[#e05252]")}>
          ...
          {txns.map((t, i) => (
            <div key={i} className="flex items-start gap-2">
              <span className={cn("mt-1 w-1 h-1 rounded-full shrink-0", t.isBuy ? "bg-[#2ec898]" : "bg-[#e05252]")} />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-1">
                  <span className="text-[10px] font-medium text-[#ede8e0] truncate">{t.name}</span>
                  <span className={cn("text-[9px] font-semibold shrink-0", t.isBuy ? "text-[#2ec898]" : "text-[#e05252]")}>
                    {t.isBuy ? "Buy" : "Sell"}
                  </span>
                </div>
