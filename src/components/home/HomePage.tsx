...
export default function HomePage() {
  ...
  const selectTicker = (t: string) => {
    ...
    window.history.pushState({}, "", `/${t.toUpperCase()}`);
    setSearchOpen(false);
    setQuery("");
    ...
    <div className="h-screen overflow-y-auto bg-[#0d1117] text-white" style={{ scrollbarWidth: "thin", scrollbarColor: "#21262d transparent" }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center gap-4">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            >
              ...
              <span className="text-xs text-[#484f58] ml-auto hidden sm:block">Search ticker</span>
            </button>

            {searchOpen && (
              <div className="absolute top-full left-0 right-0 mt-1.5 rounded-xl border border-[#30363d] bg-[#161b22] shadow-2xl shadow-black/50 overflow-hidden z-50">
          ...
          <div className="flex items-center bg-[#161b22] border border-[#21262d] rounded-lg p-0.5 shrink-0">
            <button
              ...
              className={cn("px-3 py-1 rounded-md text-xs font-medium transition-colors", activeTab === "overview" ? "bg-[#21262d] text-white" : "text-[#8b949e] hover:text-white")}
            >Overview</button>
            <button
              onClick={() => setActiveTab("discover")}
              className={cn("px-3 py-1 rounded-md text-xs font-medium transition-colors flex items-center gap-1", activeTab === "discover" ? "bg-[#21262d] text-white" : "text-[#8b949e] hover:text-white")}
            >
          ...
          <a
            href={`/${activeTicker}`}
            className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#388bfd44] bg-[#1f6feb15] text-[#388bfd] text-xs font-medium hover:bg-[#1f6feb25] hover:border-[#388bfd88] transition-colors"
          >
            <Terminal className="w-3.5 h-3.5" />
            <span className="hidden sm:block">Advanced Terminal</span>
      ...
      {activeTab === "overview" ? (
        <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 pb-24">
          ...
          {/* Main Grid: left (content) + right (insight cards) */}
